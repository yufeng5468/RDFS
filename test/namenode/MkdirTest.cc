...
//  el::Loggers::setVerboseLevel(9);
  ...
  std::string root_src = "/testing";
  std::vector<int> warmup_depths = {64};
  std::vector<int> warmup_iters = {5};
  std::vector<int> depths = {1};
  std::vector<int> num_iters = {5, 50, 100, 150, 200, 250, 500};
  ...
  for (auto d : depths) {
    for (auto i : num_iters) {
      std::cerr << "mkdir depth " << std::to_string(d) << " num_iters " << std::to_string(i) << "\n";
      for (int j = 0; j < i; j++) {
        std::string curr_src = root_src;
        for (int k = 0; k < d; k++) {
          curr_src += "/test_mkdir_r1_depth" + std::to_string(d) +
              "_num" + std::to_string(i) + "_iter" + std::to_string(j);
        }
